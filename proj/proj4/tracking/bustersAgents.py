...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for belief in livingGhostPositionDistributions:
            mostLikely = max(belief, key=belief.__getitem__) 
            ghostPositions.append(mostLikely)
        ghostpos = min(ghostPositions,
                       key=lambda pos: self.distancer.getDistance(pacmanPosition, pos))
        mindist = float("inf")
        todo = None
        for act in legal:
            newpacpos = Actions.getSuccessor(pacmanPosition, act)
            dist = self.distancer.getDistance(newpacpos, ghostpos)
            if dist < mindist:
                mindist = dist
                todo = act
        return todo
