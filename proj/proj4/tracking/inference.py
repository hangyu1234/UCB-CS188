...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for z in range(0, maxdist + MAX_NOISE + 1):
        variableDomainsDict[OBS0].append(z)
        variableDomainsDict[OBS1].append(z)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            restfactors, newfactor = joinFactorsByVariable(currentFactorsList, var)
            if newfactor is None:
                currentFactorsList = restfactors
                continue
            if len(newfactor.unconditionedVariables()) == 1 and var in newfactor.unconditionedVariables():
                currentFactorsList = restfactors
            else:
                eliminatedfactor = eliminate(newfactor, var)
                currentFactorsList = restfactors + [eliminatedfactor]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldpos in self.allPositions:
            ...
            for newpos in self.allPositions:
                result[newpos] += self.beliefs[oldpos] * newPosDist[newpos]
        result.normalize()
        self.beliefs = result
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        allpart = self.legalPositions
        for i in range(0, self.numParticles):
            self.particles.append(allpart[i % len(allpart)])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.particles:
            w = self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
