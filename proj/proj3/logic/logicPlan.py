...
def sentence2() -> Expr:
    ...
    sen3 = ~(B & ~C) >> A
    sen4 = ~D >> C
    return conjoin([sen1, sen2, sen3, sen4])
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    sen1 = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))
    sen2 = ~(PacmanAlive_0 & PacmanBorn_0)
    sen3 = PacmanBorn_0
    return conjoin([sen1, sen2, sen3])
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    sen = premise >> conclusion
    if findModel(~sen) == False:
        return True
    else:
        return False
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in range(0, len(literals)):
        for j in range(i + 1, len(literals)):
            subexprs.append(disjoin([~literals[i], ~literals[j]]))
    return conjoin(subexprs)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    moved_causes_sent = disjoin(possible_causes)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        ...
        if (x, y) in non_outer_wall_coords:
            pacman_places.append(PropSymbolExpr(pacman_str, x, y, time = t))
    pacphysics_sentences.append(exactlyOne(pacman_places))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    sen1 = conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time = 1)])
    sen2 = conjoin(KB + [~PropSymbolExpr(pacman_str, x1, y1, time = 1)])
    model1 = findModel(sen1)
    model2 = findModel(sen2)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(0, 50):
        ...
        for x, y in non_wall_coords:
            pos_t.append(PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(pos_t))
        model = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time = t)]))
        if model:
            return extractActionSequence(model, actions)
        act_t = []
        for action in actions:
            act_t.append(PropSymbolExpr(action, time = t))
        KB.append(exactlyOne(act_t))
        for x, y in non_wall_coords:
            axiom = pacmanSuccessorAxiomSingle(x, y, t + 1, walls_grid)
            if axiom:
                KB.append(axiom)
    return []
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    def foodAxioms(x, y, t):
        result = PropSymbolExpr(food_str, x, y, time = t - 1) & ~PropSymbolExpr(pacman_str, x, y, time = t - 1)
        return PropSymbolExpr(food_str, x, y, time = t) % result
    for x, y in food:
    ...
    for t in range(0, 50):
        ...
        KB.append(exactlyOne(act_t))
        for x, y in non_wall_coords:
            axiom1 = pacmanSuccessorAxiomSingle(x, y, t + 1, walls)
            if axiom1:
                KB.append(axiom1)
            axiom2 = foodAxioms(x, y, t+1)
            if axiom2:
                KB.append(axiom2)
...
def localization(problem, agent) -> Generator:
    ...
    for x, y in all_coords:
        if (x, y) in walls_list:
            has_wall.append(PropSymbolExpr(wall_str, x, y))
        else:
            no_wall.append(~PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(has_wall + no_wall))
    for t in range(agent.num_timesteps):
        if t == 0:
            KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, None))
        else:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        if t == 0:
            KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, problem.walls, sensorAxioms, None))
        else:
